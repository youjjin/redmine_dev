  ...
  module Charts
    class BurndownChart < AgileChart
      ...
      def initialize(data_scope, options = {})
        ...
        # unit -> 일감 단위로 번다운차트 보여주는 부분 색 바꾸는 부분
        @line_colors = { :work => '242,115,115', :ideal => '209,210,212', :total => '123,210,210' }
      end

      def data
        return false unless calculate_burndown_data.any?
        
        # 그래프 점선으로 할건지, 그래프 아래 채울건지에 대해서 수정하는 부분

        # datasets = [dataset(@burndown_data, l(:label_agile_actual_work_remaining), :fill => true, :dashed => true, :color => line_colors[:work])]
        ...
        #                       :color => line_colors[:total], :fill => true)
        # end
        # if @show_ideal_effort
        #   datasets << dataset(ideal_effort(@cumulative_burndown_data.first), l(:label_agile_ideal_work_remaining),
        #                       :fill => true, :color => line_colors[:ideal], :nopoints => true, :dashed => true)
        # end

        datasets = [dataset(@cumulative_burndown_data, l(:label_agile_total_work_remaining), :color => line_colors[:total], :fill => true)]
        if @show_ideal_effort
            datasets << dataset(ideal_effort(@cumulative_burndown_data.first), l(:label_agile_ideal_work_remaining),
                              :fill => true, :color => line_colors[:ideal], :nopoints => true, :dashed => true)
        end
